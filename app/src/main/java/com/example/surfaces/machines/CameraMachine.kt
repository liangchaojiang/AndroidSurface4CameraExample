...
class CameraMachine(cameraMId: String?) : StateMachine<CameraState, CameraAction> {
    ...
    override fun transition(action: CameraAction) {
        ...
        when {
            state is CameraState.WaitingStart && action is CameraAction.Start -> {
                val mutableHolder = findCameraId(state, action, cameraId)
    ...
    private fun findCameraId(
        ...
        var resultCameraId: String? = cameraId
        var resultSize: Size = Size(1920,1080)
//
...
//        }
    ...
    private fun createSession(
        ...
        if (size != null) {
            texture.rotate = Surface.ROTATION_0
